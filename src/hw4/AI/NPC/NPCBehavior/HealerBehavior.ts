...
import { AllyFilter, BattlerActiveFilter, BattlerGroupFilter, BattlerHealthFilter, BattlerLowHealthFilter, ItemFilter, RangeFilter, VisibleItemFilter } from "../../../GameSystems/Searching/HW4Filters";
...
export default class HealerBehavior extends NPCBehavior  {
    ...
    public initializeAI(owner: NPCActor, opts: Record<string, any>): void {
        ...
        pickupHealthPack.cost = 3;
        this.addState(HealerActions.PICKUP_HPACK, pickupHealthPack);

        // An action for healing an ally
        let healAlly = new UseHealthpack(this, this.owner);
        healAlly.targets = scene.getBattlers();
        healAlly.targetFinder = new BasicFinder<Battler>(null, BattlerActiveFilter(), BattlerGroupFilter([owner.battleGroup]), BattlerLowHealthFilter());
    ...
    public override update(deltaT: number): void {
        ...
        // console.log(this.owner.getScene().getBattlers().filter(BattlerActiveFilter()));
        this.stateMap.get(HealerActions.USE_HPACK).targets = this.owner.getScene().getBattlers().filter(BattlerActiveFilter());
        this.stateMap.get(HealerActions.USE_HPACK).targetFinder = new BasicFinder<Battler>(LowestHealthBattler, BattlerActiveFilter(), BattlerGroupFilter([this.owner.battleGroup]), BattlerLowHealthFilter());
        //console.log(this.stateMap.get(HealerActions.USE_HPACK).targetFinder);
    }
